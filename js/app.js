...
function renderProyecto(){
  ...
  view.innerHTML = `
    <span class="badge ready">Módulo funcional v2.5.9</span>
    <h2>📁 Proyecto</h2>
...
function renderCarpetaTecnica(){
  ...
  v.innerHTML=`<span class="badge ready">Módulo funcional v2.5.9</span>
...
function modalRic6230(){
  return `<div class="ric-modal" id="ric6230Modal"><div class="ric-modal-box">
    ...
    <p><b>GIAE v2.5.9:</b> clasifica TP, TS, equipotencialidad y sistema de tierra, dejando registro para carpeta técnica.</p>
...
function plantillaDoc251(titulo, body, e){
  ...
  <header><h1>${esc251(titulo)}</h1><p class="meta">GIAE Chile v2.5.9 · Creado por Julio Vera Concha · ${fecha}</p><p><b>Proyecto:</b> ${esc251(e.proyectoNombre||"Sin proyecto")} · <b>Cliente:</b> ${esc251(e.cliente||"Sin cliente")}</p></header>
  ${body}
...
function renderGuiaSEC252(){
 ...
 v.innerHTML=`<span class="badge ready">Módulo funcional v2.5.9</span><h2>📋 Guía SEC Inteligente · TE1 · TE2 · TE3</h2>
...
function descargarGuiaSEC252(){const e=guiaExp252(),t=getTipoDeclaracionSEC253(getP252())||"TE1";const html=`<!doctype html><html lang="es"><meta charset="utf-8"><title>Guía SEC ${t}</title><body style="font-family:Arial;margin:32px"><h1>Guía SEC ${t} · GIAE v2.5.9</h1><p><b>Proyecto:</b> ${esc252(e.proyecto)} · <b>Cliente:</b> ${esc252(e.cliente)}</p><h2>${tituloSEC252(t)}</h2><p>${descSEC252(t)}</p><h2>Pasos</h2><ol>${pasosSEC252(t)}</ol><h2>Datos desde GIAE</h2>${copiarGIAE252(e)}<h2>Pendientes</h2>${pendientesSEC252(t)}<p><small>No reemplaza plataforma oficial SEC.</small></p></body></html>`;descargarArchivo252(`GIAE_Guia_SEC_${t}_v2_5_2.html`,html,"text/html;charset=utf-8");toast("Guía SEC descargada.")}
...
function docHeader254(t,e){return`<div class="doc-print-254"><h1>${esc254(t)}</h1><p><b>GIAE Chile v2.5.9</b> · Creado por Julio Vera Concha</p><p><b>Proyecto:</b> ${esc254(e.proyecto)} · <b>Cliente:</b> ${esc254(e.cliente)} · <b>Declaración:</b> ${esc254(e.tipoSEC)}</p>`}
...
function renderExpedienteHTML258(exp){return`<div class="exp-doc-258"><h1>${esc258(exp.titulo)}</h1><p><b>GIAE Chile v2.5.9</b> · Creado por Julio Vera Concha</p>${exp.secciones.map((s,i)=>`<section><h2>${i+1}. ${esc258(s[0])}</h2><div>${s[1]}</div></section>`).join("")}</div>`}
